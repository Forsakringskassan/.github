...
Two files. Copy them as they are; every input has a default.
...
step that does not run. No inputs to set, nothing to opt out of.

| Job     | npm scripts it runs, in order                                             |
| ------- | ------------------------------------------------------------------------- |
| `build` | `build`, `test`, `integration-test`, `selftest`                           |
...
with `--ignore-scripts`, so a `pretest` that lints does not run twice.

`bundle-npm-lib-release` builds and then runs
[semantic-release](https://github.com/Forsakringskassan/semantic-release-config),
authoring the release commit as the GitHub App you pass in `app-id`.
...
| `runs-on`                         | `["ubuntu-latest"]` | JSON array of runners to build on              |
| `npm-versions`                    | `[]`                | JSON array of extra npm versions to build with |
| `npm-pkg-lint-folders`            | `.`                 | Space separated folders, for monorepos         |
| `npm-pkg-lint-allow-dependencies` | empty               | Comma separated dependencies to accept         |

`bundle-npm-lib-release`

| Input           | Default                      | Meaning                                 |
| --------------- | ---------------------------- | --------------------------------------- |
| `app-id`        | **required**                 | Client ID of the releasing GitHub App   |
| `node-version`  | `24.x`                       | Node.js version used to build           |
...
Secret: `app-key`, the private key of `app-id`. Required.

### Pinning

`@main` always works. Renovate in most repositories rewrites it to a commit SHA
with a `# main` comment and keeps it updated, which makes a change here land as
a reviewable pull request in your repository rather than silently.
...
3. Add the bundle to the table in this README.
4. `npm run lint`.
